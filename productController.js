...
async function addTowishlist(req, res, next){
    ...
    loggedInUser.wishlist.push(pid);

    await loggedInUser.save();

    // res.send('data added to cart successfully');
    res.redirect('/p1/wishlist');
}

async function showwishlist(req, res, next){
    const loggedInUser = req.user;

    const populatedUserData = await loggedInUser.populate('wishlist');
    console.log(populatedUserData);

    const wishlistData = populatedUserData.wishlist;

    console.log(wishlistData);

    res.render('product/wishlist.ejs', {wishlistData});
}

module.exports = {showAllProducts, showSingleProduct, addProductPage, createProduct, editProduct, editProductPage, deleteProduct, createReview, deleteReview, addToCart, showCart, addTowishlist, showwishlist}